...
const ContentPage = () => {
  const {
    ...
    createSliderData,
    updateSliderData,
  } = useSelector((state: any) => state?.rootReducer?.sliderReducer);
  const { allContentData, updateContentData, updateCardContentData } =
    useSelector((state: any) => state?.rootReducer?.contentSectionReducer);
  ...
  } = useSelector((state: any) => state?.rootReducer?.smallProductReducer);
  ...
  const renderClient = (params: GridRenderCellParams) => {
    ...
    if (row?.sliderImages?.length) {
      return (
        <CustomAvatar
          src={`${row?.sliderImages}`}
          sx={{ mr: 3, width: "2.575rem", height: "2.575rem" }}
        />
      );
    } else {
      return (
  ...
  // SECTION 2
  const cardData =
    allContentData && allContentData ? allContentData?.contentCards : "";
  ...
  const productCardData =
    allProductSectionData && allProductSectionData[0]
      ? allProductSectionData[0]?.bigProductContentCard
      : "";
  const productContentCardData = productCardData
    ? JSON.parse(productCardData)
    : [
        {
          productContentMainCardImage: "",
  ...
  const smallCardArray =
    getAllSmallProductData && getAllSmallProductData[0]
      ? getAllSmallProductData[0]?.smallProductContentCard
      : "";
      ...
      <Grid item xs={12}>
        ...
        >
          ...
          <AccordionDetails>
            ...
            <Box
              textAlign="end"
              sx={{ p: (theme) => theme.spacing(0, 5, 4, 5) }}
            >
              <Button
                variant="contained"
                sx={{
            ...
            <DataGrid
              ...
              autoHeight
              rows={
                allSliderData?.data && allSliderData?.data
                  ? allSliderData?.data
                  : []
              }
              columns={columns}
          ...
          <AccordionDetails>
            ...
            <Box mt={8}>
              ...
              >
                ...
                <Grid container rowSpacing={3}>
                  {JSONHandler(allContentData?.contentPointDetail)?.map(
            ...
            <Formik
              initialValues={{
                productContentMainHeading:
                  allProductSectionData[0]?.productContentMainHeading || "",
                productContentSubHeading:
                  allProductSectionData[0]?.productContentSubHeading || "",
                productContentText:
                  allProductSectionData[0]?.productContentText || "",
              }}
              enableReinitialize
              onSubmit={(values) => {
                const formData = new FormData();
                ...
                formData.append(
                  "productContentMainHeading",
                  values?.productContentMainHeading
                );
                formData.append(
                  "productContentSubHeading",
                  values?.productContentSubHeading
                );
                formData.append(
                  "productContentText",
                  values?.productContentText
                );
